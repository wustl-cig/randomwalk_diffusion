...
class NCSNpp(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.all_resolutions = all_resolutions = [
            config['imagedetail']['image_size'] // (2**i) for i in range(num_resolutions)
        ]
        ...
        channels = config['imagedetail']['num_channels']
    ...
    def forward(self, x, time_cond):
        ...
        # raise ValueError(f"self.config.data.centered: {self.config.data.centered}")
        if not self.config['imagedetail']['centered']: # 
        ...
        if self.config['model']['scale_by_sigma']: # * TRUE
